...
def call_semEP(threshold, cls_addres, file_addres):
    ...
    commd = current_path + "/" + DIR_SEM_EP + " " + file_addres + "donor.txt " + file_addres + "matrix_donor.tsv " + str(
        th)
    print('commd: ' + commd)
    os.system(commd)
    pattern = 'donor'
...
def get_measure(cls_measure, cls_folder, f_model):
    ...
    measure = '/cma ' + cls_folder + '/clusters/ ' + f_model + '/donor.txt ' + f_model + '/matrix_sim.txt'
...
for m in model_list:
    ...
    df_donor['response'] = 'non_cured'
    df_donor.loc[df_donor.donor.isin(target.donorID), 'response'] = 'cured'
    ...
    for th in threshold:
        ...
        sim_matrix, percentile, list_sim = Utility.matrix_similarity(df_donor.drop(columns=['response']),
                                                                     Utility.cosine_sim,
        ...
        for file in entries:
            sim_matrix.loc[
                sim_matrix.index.isin(Utility.load_cluster(file, cls_address + 'clusters/')), 'cluster'] = int(
                file[:-4].split('-')[1])
            df_donor.loc[
                df_donor.donor.isin(Utility.load_cluster(file, cls_address + 'clusters/')), 'cluster'] = int(
                file[:-4].split('-')[1])
        ...
        if len(entries) < 9:
            new_df = Utility.plot_semEP(len(entries), sim_matrix, path_plot, 'PCA_th_' + str(th) + 'matrix.pdf',
                                        scale=False)
            new_df[['response', 'cluster']].to_csv(path_plot + 'th_' + str(th) + '_summary.csv')
            Utility.plot_semEP(len(entries), df_donor.drop(columns=['donor']), path_plot, 'PCA_th_' + str(th) + '.pdf',
                               scale=False)
